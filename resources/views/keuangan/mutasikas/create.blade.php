...
<div class="container" style="max-width:600px;">
    <div class="card">
        <form action="{{ route('mutasikas.store') }}" method="POST">
            @csrf
            <div class="card-body">
                <div class="row g-3 mb-3">
                    <div class="col-sm-6">
                        ...
                        <select name="dari" id="dari" required class="form-select">
                            ...
                            @foreach($kasList as $k)
                                <option value="{{ $k->kode }}" {{ old('dari') == $k->kode ? 'selected' : '' }}>
                                    {{ $k->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="ke" id="ke" required class="form-select">
                            <option value="">-- Pilih Kas --</option>
                            @foreach($kasList as $k)
                                <option value="{{ $k->kode }}" {{ old('ke') == $k->kode ? 'selected' : '' }}>
                                    {{ $k->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="alert alert-warning py-2 small">
                    <i class="fa-solid fa-circle-info me-1"></i> Kas tujuan harus berbeda dengan kas asal
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                              class="form-control" placeholder="Catatan mutasi kas...">{{ old('keterangan') }}</textarea>

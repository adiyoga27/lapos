            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <select name="kategori_id" id="kategori_id" required class="form-select">
                        ...
                        @foreach($kategori as $kat)
                            <option value="{{ $kat->id }}" {{ old('kategori_id', $pemasukan->kategori_id) == $kat->id ? 'selected' : '' }}>
                                {{ $kat->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="kas_id" id="kas_id" required class="form-select">
                        ...
                        @foreach($kas as $k)
                            <option value="{{ $k->id }}" {{ old('kas_id', $pemasukan->kas_id) == $k->id ? 'selected' : '' }}>
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $pemasukan->jumlah) }}" required
                ...
                <div class="mb-3">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                              class="form-control" placeholder="Catatan pemasukan...">{{ old('keterangan', $pemasukan->keterangan) }}</textarea>
                    @error('keterangan') <p class="text-danger small mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="mb-3">
                    <label for="tanggal" class="form-label">Tanggal</label>
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', \Carbon\Carbon::parse($pemasukan->tanggal)->format('Y-m-d')) }}" required class="form-control">

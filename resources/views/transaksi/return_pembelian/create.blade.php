...
@section('title') Return Pembelian Baru @endsection
@section('content')
<div x-data="returnPembelianScreen()" x-init="init()" class="container" style="max-width:960px;">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <h6 class="border-bottom pb-2 mb-3">Pilih Faktur Pembelian</h6>
            <div class="position-relative">
                ...
                <div x-show="showFakturDropdown && fakturResults.length > 0" x-cloak
                    ...
                    <template x-for="f in fakturResults" :key="f.kode">
                        <div @click="pilihFaktur(f); showFakturDropdown = false"
                            ...
                            <p class="mb-0 text-muted" style="font-size:0.75rem;">
                                <span x-text="formatDate(f.tanggal)"></span> |
                                <span x-text="f.supplierRef ? f.supplierRef.nama : '-'"></span> |
            ...
            <div x-show="selectedFaktur" class="mt-3 alert alert-info py-2 px-3 small">
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <span class="text-muted" style="font-size:0.75rem;">Supplier</span>
                        <p class="fw-medium mb-0" x-text="selectedFaktur && selectedFaktur.supplierRef ? selectedFaktur.supplierRef.nama : '-'"></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card mb-3" x-show="purchaseItems.length > 0">
        ...
        <div class="table-responsive">
            <table class="table table-sm table-hover small mb-0">
                ...
                <tbody>
                    <template x-for="(item, index) in purchaseItems" :key="index">
                        <tr>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="form-label small fw-medium text-muted">Kas (Penerimaan)</label>
            ...
            <div class="mt-3">
                ...
                <textarea x-model="alasan" rows="2" class="form-control form-control-sm" placeholder="Alasan return ke supplier..."></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('return_pembelian.index') }}" class="btn btn-danger flex-fill fw-bold d-flex align-items-center justify-content-center gap-2">
...
<script>
    function returnPembelianScreen() {
        return {
            fakturSearch: '',
            fakturResults: [],
            ...
            async searchFaktur() {
                if (this.fakturSearch.length < 1) {
                    this.fakturResults = [];
                    this.showFakturDropdown = false;
                    return;
                }
                try {
                    const url = '/pembelian/ajax/faktur/' + encodeURIComponent(this.fakturSearch);
                    const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
                    const data = await res.json();
                    if (data.success) {
                        this.fakturResults = data.data;
                        this.showFakturDropdown = true;
                    }
                } catch (e) {
                    this.fakturResults = [];
                }
            ...
            async pilihFaktur(faktur) {
                ...
                this.showFakturDropdown = false;
                try {
                    const url = '/return_pembelian/get-items/' + encodeURIComponent(faktur.kode);
                    const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
                    const data = await res.json();
                    if (data.success) {
                        this.purchaseItems = data.data.map(function(item) {
                            item.qty_return = 0;
                            return item;
                        });
                    }
                } catch (e) {
                    this.showFlash('Gagal mengambil item pembelian', 'error');
                }
            },

            recalcAll() {
                var self = this;
                this.totalReturn = this.purchaseItems.reduce(function(sum, item) {
            ...
            async prosesReturn() {
                var self = this;
                var items = this.purchaseItems.filter(function(item) { return (item.qty_return || 0) > 0; });
                if (items.length === 0) {
                    this.showFlash('Pilih item yang akan direturn', 'error');
                    return;
                }
                if (!this.selectedFaktur) {
                    this.showFlash('Pilih faktur pembelian', 'error');
                    return;
                }
                ...
                try {
                    var payload = {
                        ...
                        supplier: this.selectedFaktur.supplier,
                    ...
                    var res = await fetch('{{ route("return_pembelian.store") }}', {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.success) {
                        ...
                        setTimeout(function() { window.location.href = '/return_pembelian/' + data.kode; }, 1000);
                    } else {

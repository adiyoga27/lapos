...
<a href="{{ route('satuan.create') }}" class="btn btn-primary btn-sm"><i class="fa-solid fa-plus"></i> Tambah</a>
</div>
<div class="card-body">
...
<li><a class="dropdown-item" href="{{ route('satuan.show', $item->nama) }}"><i class="fa-solid fa-eye me-2"></i>Detail</a></li>
<li><a class="dropdown-item" href="{{ route('satuan.edit', $item->nama) }}"><i class="fa-solid fa-pen-to-square me-2"></i>Edit</a></li>
<li><hr class="dropdown-divider"></hr></li>
<li><form action="{{ route('satuan.destroy', $item->nama) }}" method="POST" onsubmit="return confirm('Hapus satuan ini?')">@csrf @method('DELETE')<button type="submit" class="dropdown-item text-danger"><i class="fa-solid fa-trash me-2"></i>Hapus</button></form></li>
</ul></div>
</td><td>{{ $item->nama }}</td></tr>@endforeach
